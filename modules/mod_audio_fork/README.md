...
```
uuid_audio_fork <uuid> start <wss-url> <mix-type> <sampling-rate> <streamID> <accID> <callSid> <track>  <token> <metadata>
```
...
- `authToken` - if token is present it will lws will pass it in th authorization header of the connection request.    
